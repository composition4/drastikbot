...
class Main():
    def __init__(self, conf_dir, debug):
        ...
        self.cap_ack   = []    # List of capabilities ACKnowledged by the server
    ...
    def text_work(self): # ^^^consider rewriting and using regex (tho it may be slower)
        ...
        try: self.irc_cmd = self.irc_cmd_list[1] # PRIVMSG, NOTICE, MODE etc.
        except IndexError: self.irc_cmd = self.irc_cmd_list[0] # PING etc.
        try: self.channel = self.irc_cmd_list[2] # IRC channel or bot nickname
        except IndexError: self.channel  = '' 
        if self.channel == self.irc.nickname: self.channel = self.usernick # for PMs
        try: self.txtmsg = self.bufflist[1]
    ...
    def service(self):
        while self.connected:
            ...
            if 'PRIVMSG' == self.irc_cmd:
                self.thread_make(self.mod.mod_main,(self.irc, self.info, self.command))
    ...
    def sasl(self): # ^^^change to use self.text_work() for more precision
        ...
        while True:
            ...
            if 'AUTHENTICATE +' in buff:
                sasl_pass = '{}\0{}\0{}'.format(self.irc.username, self.irc.username, self.irc.auth_password)
                self.irc.send(('AUTHENTICATE', base64.b64encode(sasl_pass.encode('utf-8'))))
    ...
    def nickserv_id(self):
        self.irc.privmsg('NickServ','IDENTIFY {} {}'.format(self.irc.nickname, self.auth_pass))
            
    def register(self):
        self.irc.send(('CAP', 'LS', self.ircv3_ver))
        self.irc.send(('USER', self.irc.username, '0', '*', ':{}'.format(self.irc.realname)))
        self.irc.nick(self.irc.nickname)
        ...
        while True:
            ...
            if ' LS :' in self.buff:
                self.ircv3 = True
                self.cap_list = re.search(r"(?:CAP .* LS :)(.*)", self.buff).group(1).split(' ')
            ...
            if 'Nickname is already in use' in self.buff: # ^^^replace self.buff with more specific
                ...
                if self.auth and self.auth_pass:
                    self.irc.privmsg('NickServ','GHOST {} {}'.format(self.irc.nickname, self.irc.auth_password))
                    #self.irc.nick(self.irc.nickname) ^^^maybe removing makes it work, investigate
            else:
                self.nickname_ = False
        ...
        if (self.irc.authentication.lower() == 'sasl' and not self.ircv3) or (self.irc.authentication.lower() == 'sasl' and 'sasl' not in self.cap_list):
            print(' - [WARNING] SASL is not supported by the server, use NickServ instead. Exiting...')
