...
class Drastikbot():
    ...
    def __init__(self, conf_dir, debug):
        ...
        self.authentication = config['irc'][
            'connection']['authentication'] or 'None'
    ...
    def send(self, cmds, text=None):  # textFix stuff and send them
        ...
        if text:
            ...
            # With CR-LF we are left with 510 characters to use, hence the
            # [:510]
            toSend = (' '.join(cmds) + ' :' + text)[
                :510] + '\r\n'  # for text with spaces
        else:
            toSend = ' '.join(cmds)[:510] + '\r\n'  # for commands
        self.irc_sock.send(toSend.encode('utf-8'))
        if self.debug:
            print(toSend.encode('utf-8'))
